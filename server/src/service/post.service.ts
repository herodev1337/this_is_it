...
export async function hasUserPostLiked(userId: string, postId: string):Promise<Boolean>{
    let userHasLiked = await Post.find({_id: postId }).countDocuments({likes: new ObjectId(userId)})
    return +userHasLiked > 0 ? true : false
}

export async function likePost(userId: string, postId: string){
    return Post.findOne({ _id: postId }).update({ $push: { likes: new ObjectId(userId)}})
}

export async function dislikePost(userId: string, postId: string){
    return Post.findOne({ _id: postId }).update({ $pull: { likes: new ObjectId(userId)}})
}